          ...
          {HEADING_STYLES.map(hs => {
            ...
            return (
              <button key={hs.id} onClick={() => set('headingStyle', hs.id)}
                className={`p-2.5 rounded-xl border-2 text-center transition ${sel ? CHIP_ON : CHIP_OFF}`}>
                <p className={`text-xs font-semibold ${sel ? 'text-pink-700 dark:text-pink-300' : 'text-gray-600 dark:text-[var(--text-secondary)]'}`}>{hs.label}</p>
          ...
          {SECTION_SPACINGS.map(ss => {
            ...
            return (
              <button key={ss.id} onClick={() => set('sectionSpacing', ss.id)}
                className={`p-2.5 rounded-xl border-2 text-center transition ${sel ? CHIP_ON : CHIP_OFF}`}>
                <p className={`text-xs font-semibold ${sel ? 'text-pink-700 dark:text-pink-300' : 'text-gray-600 dark:text-[var(--text-secondary)]'}`}>{ss.label}</p>
          ...
          {SHADOW_INTENSITIES.map(si => {
            ...
            return (
              <button key={si.id} onClick={() => set('shadowIntensity', si.id)}
                className={`p-2.5 rounded-xl border-2 text-center transition ${sel ? CHIP_ON : CHIP_OFF}`}>
                <p className={`text-xs font-semibold ${sel ? 'text-pink-700 dark:text-pink-300' : 'text-gray-600 dark:text-[var(--text-secondary)]'}`}>{si.label}</p>
