    ...
    <div className="flex items-center gap-3 px-4 py-3.5 hover:bg-[var(--surface-hover)] transition group">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          ...
          <button
            onClick={copy}
            title="Copy"
            className="p-1 rounded text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-hover)] transition focus-visible:outline-none focus-visible:shadow-[var(--focus-ring)]"
          >
            {copied ? (
        ...
        <div className="flex items-center gap-2 mt-0.5">
          ...
          {(link.tags ?? []).slice(0, 3).map((t) => (
            <span
              key={t}
              className="text-[10px] px-1.5 py-0.5 rounded-[var(--radius-sm)] bg-[var(--surface-muted)] text-[var(--text-tertiary)]"
            >
              #{t}
            </span>
          ))}
      ...
      <div className="flex items-center gap-1 text-sm shrink-0">
        <span className="font-semibold text-[var(--text-primary)]">
          {link.click_count.toLocaleString("en-IN")}
        </span>
      ...
      <Link
        ...
        className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium border border-[var(--border)] bg-[var(--surface-muted)] text-[var(--text-secondary)] hover:bg-[var(--surface-hover)] hover:text-[var(--text-primary)] rounded-[var(--radius-sm)] transition shrink-0 focus-visible:outline-none focus-visible:shadow-[var(--focus-ring)]"
      >
      ...
      <div className="flex items-center gap-0.5 shrink-0">
        <button
          onClick={() => onEdit(link)}
          className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium border border-[var(--border)] bg-[var(--surface-muted)] text-[var(--text-secondary)] hover:bg-[var(--surface-hover)] hover:text-[var(--text-primary)] rounded-[var(--radius-sm)] transition focus-visible:outline-none focus-visible:shadow-[var(--focus-ring)]"
        >
          ...
          createPortal(
            <>
              ...
              >
                ...
                />
                <MenuItem
                  icon={link.archived_at ? ArchiveRestore : Archive}
                  label={link.archived_at ? "Unarchive" : "Archive"}
                  onClick={() => {
                    setMenu(false);
                    onArchive(link);
                  }}
