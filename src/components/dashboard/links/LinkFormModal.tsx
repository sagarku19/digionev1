...
export function LinkFormModal({
  ...
  useEffect(() => {
    if (!open) return;
    // Prop→state sync: reset the form from `editing` each time the modal opens.
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-lg my-4 sm:my-8 flex flex-col max-h-[calc(100vh-2rem)] sm:max-h-[calc(100vh-4rem)] bg-[var(--surface)] border border-[var(--border)] rounded-[var(--radius-lg)] shadow-[var(--shadow-lg)]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-subtle)] shrink-0">
          <h2 className="text-lg font-semibold text-[var(--text-primary)]">{editing ? 'Edit link' : 'New short link'}</h2>
          <button
            onClick={onClose}
        ...
        {/* Footer */}
        <div className="p-6 border-t border-[var(--border-subtle)] bg-[var(--surface-muted)] shrink-0">
          <button
            onClick={submit}
