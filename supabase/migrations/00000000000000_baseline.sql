-- ============================================================================
-- DigiOne — BASELINE schema migration (public schema)
-- ============================================================================
-- Generated from the live database (project qcendfisvyjnwmefruba) via pg_dump.
-- This recreates EVERY enum, table, sequence, index, constraint, function, and
-- trigger in the `public` schema on a fresh/empty Supabase project.
--
-- Apply order on a new project:
--   1. this file (baseline)
--   2. 20260418_insta_autodm.sql      (NOT yet applied to live DB — pending feature)
--   3. supabase/seed.sql              (reference data)
--   4. supabase/exports/storage-and-auth.sql  (buckets, storage RLS, auth triggers)
--
-- The 20260416_marketing_tables.sql migration is ALREADY contained in this
-- baseline (community_posts, community_reactions, services, service_bookings are
-- live). It is kept for history and is a safe no-op (uses IF NOT EXISTS).
--
-- NOTE on idempotency: pg_dump emits CREATE TYPE / CREATE TABLE without
-- IF NOT EXISTS. This file is meant to run ONCE on an empty database (the
-- `supabase db reset` / new-project model). To re-run on a dirty DB, drop the
-- public schema first (see REBUILD.md), or run via `supabase db reset`.
--
-- NOTE on auth triggers: handle_new_user() and handle_user_email_confirmed()
-- are created here, but their TRIGGER bindings live on auth.users (outside the
-- public schema) and are in supabase/exports/storage-and-auth.sql. Without that
-- file, signup will NOT create public.users / public.profiles rows.
-- ============================================================================
...
-- (public schema already exists on Supabase)
