...
Quick-reference for every public table. Data sourced from the live Supabase DB (`qcendfisvyjnwmefruba`) on 2026-07-07 (post Phases 0–6: refunds, tax engine, invoices).
...
All 68 tables have RLS enabled. The "Policies" count is the number of distinct `pg_policies` rows; 0 means RLS is on but no explicit policies exist (effectively blocks all access unless using service role).
...
| `balance_reconciliation_log` | Drift log written by `reconcile_creator_balances` (alert-only, never auto-corrects) | `creator_id` | on / 1 | `reconcile_creator_balances` RPC (service-role) |
...
| `creator_balances` | Running balance ledger per creator: `total_earnings` (gross), `total_platform_fees`, `total_paid_out`, `pending_payout`, `frozen_balance` | `creator_id` | on / 2 | service-role RPCs only (`credit_creator_balance`, `settle_payout`, `begin_refund`/`settle_refund`, freeze/release) |
| `creator_kyc` | KYC submission and verification status per creator (PAN/bank/UPI encrypted at rest; GSTIN for GST-registered creators) | `creator_id` | on / 2 | `POST /api/kyc/submit` + `/api/kyc/gstin` (service-role); verify via `scripts/kyc-admin.ts` |
...
| `guest_entitlements` | Email-keyed purchase grants for guest buyers, claimed on later sign-in | — (claimed via verified JWT email) | on / 1 | fulfillment (service-role); claimed by `/api/account/claim-entitlements` |
| `guest_leads` | Anonymous visitor contact captures (pre-auth) | `site_id` → `sites.creator_id` | on / 1 | `/api/leads` (service-role) |
| `invoice_counters` | Atomic per-series invoice numbering (`series_key`, `fy`, `last_number`) | — | on / 0 | `issue_invoice` RPC (service-role) — no policies by design |
| `invoices` | Issued-invoice registry with frozen content snapshot (sale bill-of-supply + commission tax invoices) | `creator_id` | on / 1 | `issue_invoice` RPC via `/api/invoices/*` (service-role) |
| `kyc_access_log` | Audit row per admin KYC-doc signed-URL mint | — (super_admin reads) | on / 1 | `/api/admin/kyc/[creatorId]/download` (service-role) |
| `kyc_documents` | Links uploaded KYC proof files (`storage_files`) to a creator by doc type | `creator_id` | on / 2 | `POST /api/kyc/documents` (service-role) |
...
| `refunds` | Refund records — freeze → Cashfree PG refund → webhook-settled clawback | `creator_id` | on / 1 | `begin_refund`/`settle_refund` RPCs via `/api/refunds/create` + PG webhook (service-role) |
...
| `storage_files` | R2 object metadata (bucket + key + mime + size + kind incl. `invoice`/`tax_doc`) | `owner_id` | on / 2 | `/api/upload/confirm`, `/api/media/*`, invoice/statement routes (service-role) |
| `subscription_plans` | DigiOne platform subscription plan definitions (free/plus/pro, `platform_fee_percent`) | — (authenticated read) | on / 1 | admin / seed scripts |
| `subscriptions` | Creator's active plan — snapshotted `current_platform_fee_percent` tiers the money path | `creator_id` | on / 1 | `activateSubscription` via `scripts/subscription-admin.ts` (service-role) |
| `tax_rules` | Versioned tax rates/thresholds (GST 18%, TDS 0.1%/5%, TCS 0.5%, ₹5L/₹20L) — snapshotted per sale | — (authenticated read) | on / 1 | service-role (seeded; never edited in place) |
| `tax_transactions` | Immutable per-sale tax accrual (commission split, pending TDS/TCS, reversal counter-rows) | `creator_id` | on / 1 | `record_sale_tax`/`begin_payout_tax`/`settle_*` RPCs (service-role) |
...
| `wallet_frozen_logs` | Audit of `frozen_balance` holds (refund clawbacks, dispute/manual freezes) | `creator_id` | on / 1 | `begin_refund`/`settle_refund`/`freeze_creator_funds`/`release_frozen_funds` RPCs (service-role) |
...
| `creator_payouts` | Payout record; written by `/api/payouts/request` after KYC + balance checks; carries `tds_withheld`/`tcs_withheld`/`net_amount` (Phase 5) |
| `refunds` | Refund lifecycle; only `begin_refund`/`settle_refund` RPCs may write |
| `wallet_frozen_logs` | Frozen-balance audit; only the freeze/release/refund RPCs may write |
| `tax_transactions` | Immutable tax accrual; only `record_sale_tax`/`begin_payout_tax`/`settle_*` RPCs may write |
| `invoices` | Legal invoice registry (frozen snapshots); only the `issue_invoice` RPC may write |

## Tables with 0 RLS policies

`rate_limits` and `invoice_counters` have RLS enabled but no policies. This is intentional: `rate_limits` is touched only by the `check_rate_limit` RPC, and `invoice_counters` only by the `issue_invoice` RPC (both `SECURITY DEFINER`, service-role). Direct table access via any key other than service-role is blocked.
