        ...
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[var(--radius-lg)] p-5 space-y-3">
          ...
          <div className="relative">
            <button
              ...
              className="w-full flex items-center justify-between px-3 py-2.5 text-sm bg-[var(--surface-muted)] border border-[var(--border)] rounded-[var(--radius-md)] text-[var(--text-primary)] focus:border-[var(--border-strong)] focus-visible:outline-none focus-visible:shadow-[var(--focus-ring)] transition"
            >
            ...
            {showProvider && (
              <div className="absolute top-full mt-1 w-full bg-[var(--surface)] border border-[var(--border)] rounded-[var(--radius-md)] shadow-[var(--shadow-sm)] z-10 overflow-hidden">
                {PROVIDERS.map(p => (
                  <button key={p} onClick={() => { setProvider(p); setShowProvider(false); }}
                    className="w-full px-4 py-2.5 text-sm text-left text-[var(--text-primary)] hover:bg-[var(--surface-hover)] transition-colors focus-visible:outline-none focus-visible:shadow-[var(--focus-ring)]">
                    {p}
                  </button>
                ))}
              </div>
        ...
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[var(--radius-lg)] p-5">
          ...
          <div className="space-y-2">
            {sequences.map(seq => (
              <div
                key={seq.id}
                className={`flex items-center gap-4 p-4 rounded-[var(--radius-md)] border-2 transition-all ${
                  seq.active
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <span className="text-[10px] font-semibold text-[var(--info)] bg-[var(--info-bg)] px-1.5 py-0.5 rounded-[var(--radius-sm)] mt-1 inline-block">
                    {seq.trigger}
                  </span>
                </div>
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus-visible:outline-none focus-visible:shadow-[var(--focus-ring)] ${
                    seq.active ? 'bg-[var(--info)]' : 'bg-[var(--border)]'
                  }`}
                >
                  <span
