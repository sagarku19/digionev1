...
import {
  ...
  Edit2, Link as LinkIcon, Copy, Check,
} from 'lucide-react';
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        duration_minutes: form.service_type === '1on1' ? Number(form.duration_minutes) : null,
        is_active: true,
    ...
    <>
      <div className="space-y-5 pt-4 pb-20">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Monetize your time with 1:1 calls, retainers, and audits</p>
          </div>
        ...
        {/* Tab switcher */}
        <div className="flex items-center gap-1 p-1 bg-gray-100 dark:bg-gray-900 rounded-xl w-fit">
          {(['services', 'bookings'] as const).map(t => (
            <button key={t} onClick={() => setActiveTab(t)}
              className={`px-4 py-1.5 rounded-lg text-xs font-bold capitalize transition ${
                activeTab === t ? 'bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-400'
              }`}>
        ...
        {loading ? (
          ...
          services.length === 0 ? (
            <div className="flex flex-col items-center py-24 text-center bg-white dark:bg-[#0A0A1A] border border-gray-200 dark:border-gray-800 rounded-2xl">
              ...
              <p className="font-semibold text-gray-800 dark:text-gray-200 mb-1">No services yet</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {services.map(svc => {
                ...
                return (
                  <div key={svc.id} className="bg-white dark:bg-[#0A0A1A] border border-gray-200 dark:border-gray-800 rounded-2xl p-5 hover:border-gray-300 dark:hover:border-gray-700 transition group flex flex-col">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="flex items-center gap-1">
                        <button onClick={() => openEdit(svc)}
                          className="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition opacity-0 group-hover:opacity-100">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className="text-sm font-bold text-gray-900 dark:text-white truncate">{svc.title}</h3>
                      ...
                      <div className="flex items-center gap-3 text-xs text-gray-600 dark:text-gray-400 mb-3">
                        <span className="font-bold text-gray-900 dark:text-white text-sm">₹{svc.price}</span>
          ...
          bookings.length === 0 ? (
            ...
            <div className="bg-white dark:bg-[#0A0A1A] border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden">
              <div className="px-5 py-3.5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                <h2 className="text-sm font-bold text-gray-900 dark:text-white">All Bookings</h2>
              ...
              <div className="divide-y divide-gray-100 dark:divide-gray-800">
                {bookings.map(bkg => {
                  ...
                  return (
                    <div key={bkg.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-gray-50 dark:hover:bg-gray-900/40 transition">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">{bkg.customer_name ?? bkg.customer_email ?? '—'}</p>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-[#0D0D1F] rounded-2xl shadow-2xl w-full max-w-lg border border-gray-200 dark:border-gray-800 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800 sticky top-0 bg-white dark:bg-[#0D0D1F] z-10">
              <div className="flex items-center gap-2.5">
                ...
                <h2 className="text-base font-bold text-gray-900 dark:text-white">{editService ? 'Edit Service' : 'New Service'}</h2>
              </div>
                  ...
                  {SERVICE_TYPES.map(t => (
                    <button key={t.id} type="button" onClick={() => setForm(p => ({ ...p, service_type: t.id }))}
                      className={`flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition ${
                        form.service_type === t.id ? `border-indigo-500 ${t.bg}` : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'
                      }`}>
                      <t.icon className={`w-5 h-5 ${form.service_type === t.id ? t.color : 'text-gray-400'}`} />
                      <span className={`text-xs font-semibold ${form.service_type === t.id ? t.color : 'text-gray-500 dark:text-gray-400'}`}>{t.label}</span>
                    </button>
                  ))}
                </div>
              </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#0D0E1A] border border-gray-200 dark:border-gray-800 rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            ...
            <h3 className="font-bold text-gray-900 dark:text-white mb-1">Delete service?</h3>
