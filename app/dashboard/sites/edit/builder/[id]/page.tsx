'use client';
// Edit page: Builder App — visual editor with sections, products, and live preview.
...
export default function EditBuilderPage() {
  ...
  const handleTypeSave = async () => {
    ...
    if (configExists) {
      ...
      await supabase.from('site_sections_config').insert({ site_id: siteId, site_type: 'builder', sections: sections as any });
    }
        ...
        <div className="space-y-5">
          <div className="flex gap-1 bg-gray-100 dark:bg-gray-900 p-1 rounded-xl">
            {[
              ...
              { id: 'info' as const,     label: 'App Info', icon: Hammer },
            ].map(t => (
              <button
                key={t.id}
          ...
          {contentTab === 'info' && (
            <div className="space-y-5">
              <div className="bg-white dark:bg-[#0A0A1A] border border-gray-200 dark:border-gray-800 rounded-2xl p-5 space-y-4">
                <div>
                  <h3 className="text-sm font-semibold text-gray-900 dark:text-white">App Info</h3>
                  <p className="text-xs text-gray-500 mt-0.5">Basic details about your builder app</p>
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1.5">App Name</label>
                  <input type="text" value={title} onChange={e => setTitle(e.target.value)} className={INPUT} placeholder="e.g. My Portfolio" />
                </div>
                <div>
                  ...
                  <textarea rows={2} value={description} onChange={e => setDescription(e.target.value)}
                    className={`${INPUT} resize-none`} placeholder="What are you building?" />
                </div>
              </div>
            </div>
          )}
