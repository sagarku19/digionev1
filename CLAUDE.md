...
| `.claude/todo-later/8(left)-2026-06-15-dashboard-health-findings.md` | **Dashboard health-check findings** (read-only sweep; storefront was clean): the `media` page targets the dropped `uploads` bucket (likely prod-broken), an `autodm` rules-of-hooks violation (non-wired prototype), and the ~81 site-edit `any` lint errors (cross-refs todo-later 7). Read when picking up dashboard hardening or asking "what's wrong in the dashboard?". |
| `.claude/todo-later/9(done)-2026-06-25-cloudflare-r2-storage-migration.md` | **Cloudflare R2 storage migration — decision record (ALL decisions resolved, not started).** Move file storage Supabase→R2 (egress is free); **storage only, the DB schema is NOT being rewritten** — keep existing tables. Decided: 4 R2 buckets (dedicated write-only `digione-kyc-private`), a new RLS'd `files` metadata table (replaces the `sum_bucket_bytes_for_prefix` quota RPC) + mandatory `kyc_access_log`, a thin `src/lib/storage/` provider abstraction, `sharp` for image conversion, dev big-bang cutover (no migration-safety apparatus), new deps `@aws-sdk/client-s3` + `s3-request-presigner` (approved). **Read before any storage / `/api/upload` / `/api/deliverables` / `/api/private/download` / bucket work** — it supersedes the Supabase-Storage assumptions in `supabase-reference.md` & `api-routes.md` once implemented. |
| `.claude/todo-later/10(left)-2026-06-29-discover-performance-and-product-metrics.md` | **`/discover` perf + real product metrics (decisions locked, not started).** Part A (pure front-end, no schema): Cloudflare Image Resizing thumbnails (`/cdn-cgi/image/`), infinite-scroll pagination (page 24), cheaper card paint. Part B: a real popularity/metrics pipeline (`product_events` + `product_stats` + `increment_product_stat` RPC + pg_cron `trending_score`) to replace the fake "Trending" sort and power creator analytics — the marketplace event→aggregate pattern, mirroring `linkinbio_analytics`. Read before touching `/discover`, product ranking, or product analytics. |
